...
[_QWERTY] = LAYOUT( \
  ...
  [_US] = LAYOUT( \
       ...
       KC_Q,              KC_W,  KC_E,  KC_R,  KC_T,             KC_Y,  KC_U,  KC_I,       KC_O,       KC_P,\
       LT(_ADJUST,KC_A),  KC_S,  KC_D,  KC_F,  KC_G,             KC_H,  KC_J,  KC_K,       KC_L,       TD(USCCO),\
       KC_Z,              KC_X,  KC_C,  KC_V,  KC_B,             KC_N,  KC_M,  TD(CO_LT),  TD(DO_GT),  OSM(MOD_LSFT),\
            TD(LA_TA),  LT(_USLOWER,KC_BSPC),  CTL_T(KC_ESC),      LGUI_T(KC_TAB),  LT(_RAISE,KC_SPC),  TD(LA_ES) \
  ),

  [_USEucalyn] = LAYOUT( \
      _____, _____,  TD(CO_LT),  TD(DO_GT),  TD(USCCO),      KC_M,  KC_R,  KC_D,  KC_Y, _____,\
      _____,  KC_O,  KC_E,       KC_I,       KC_U,           KC_G,  KC_T, _____,  KC_S,  KC_N,\
      _____, _____,  _____,      _____,      KC_F,           KC_B,  KC_H,  KC_J,  KC_L, _____,\
                                   _____, _____, _____,      _____, _____, _____ \
  ),
  ...
  [_USLOWER] = LAYOUT( \
       TD(UF1EX),    TD(UF2AT),  TD(UF3HA),  TD(UF4DL),   TD(UF5PE),       KC_CALC,     KC_7,  KC_8,  KC_9,  TD(UQUDQ),\
       TD(UF6CI),    TD(UF7AM),  TD(UF8AS),  TD(UF9LRP),  TD(UF10LR),      TD(UEQPL),  KC_4,  KC_5,  KC_6,  TD(UMIUM),\
       TD(F11_F12),  TD(UGRTI),  TD(UBSPI),  TD(ULPRP),   TD(ULBRB),       KC_0,       KC_1,  KC_2,  KC_3,  TD(SL_QU),\
                                             TD(SF_TA), _____, _____,      _____,KC_ENT, _____ \
  ),

  [_RAISE] = LAYOUT( \
      DMR2,     DMR1,     DMP2,       DMP1,    DMRS,        XXXXX,  KC_HOME,  KC_PGUP,  KC_END,   KC_MUTE,\
  ...
  [_ADJUST] = LAYOUT( \
      ...
      XXXXX,             XXXXX,      XXXXX,  KC_ACL2,  KC_ACL1,      XXXXX,  TO(_QWERTY), TO(_US),    XXXXX,    _____,\
...
//US配列用
//Define Tap dance -> UF9LRP
void dance_Uf9lrp_finished (qk_tap_dance_state_t *state, void *user_data) {
	if (state->count == 1) {
		register_code (KC_F9);
	} else {
		register_code (KC_LSFT);
...
void dance_Uf9lrp_reset (qk_tap_dance_state_t *state, void *user_data) {
	if (state->count == 1) {
		unregister_code (KC_F9);
	} else {
		unregister_code (KC_LSFT);
...
//Define Tap dance -> UF10LR
void dance_Uf10lr_finished (qk_tap_dance_state_t *state, void *user_data) {
	if (state->count == 1) {
		register_code (KC_F10);
	} else {	
...
void dance_Uf10lr_reset (qk_tap_dance_state_t *state, void *user_data) {
	if (state->count == 1) {
		unregister_code (KC_F10);
	} else {
...
qk_tap_dance_action_t tap_dance_actions[] = {
	...
	[UGRTI]   = ACTION_TAP_DANCE_DOUBLE(KC_GRV,  KC_TILD), //input ` ~ ここからUS配列
	[UF1EX]   = ACTION_TAP_DANCE_DOUBLE(KC_F1,   KC_EXLM), //input F1 !
	[UF2AT]   = ACTION_TAP_DANCE_DOUBLE(KC_F2,   KC_AT),   //input F2 @
	[UF3HA]   = ACTION_TAP_DANCE_DOUBLE(KC_F3,   KC_HASH), //input F3 #
	[UF4DL]   = ACTION_TAP_DANCE_DOUBLE(KC_F4,   KC_DLR),  //input F4 $
	[UF5PE]   = ACTION_TAP_DANCE_DOUBLE(KC_F5,   KC_PERC), //input F5 %
	[UF6CI]   = ACTION_TAP_DANCE_DOUBLE(KC_F6,   KC_CIRC), //input F6 ^
	[UF7AM]   = ACTION_TAP_DANCE_DOUBLE(KC_F7,   KC_AMPR), //input F7 &
	[UF8AS]   = ACTION_TAP_DANCE_DOUBLE(KC_F8,   KC_ASTR), //input F8 *
	[ULPRP]   = ACTION_TAP_DANCE_DOUBLE(KC_LPRN, KC_RPRN), //input ( )
	[ULBRB]   = ACTION_TAP_DANCE_DOUBLE(KC_LBRC, KC_RBRC), //input [ ]
	[UEQPL]   = ACTION_TAP_DANCE_DOUBLE(KC_EQL,  KC_PLUS), //input = +
	[UMIUM]   = ACTION_TAP_DANCE_DOUBLE(KC_MINS, KC_UNDS), //input - _
	[USCCO]   = ACTION_TAP_DANCE_DOUBLE(KC_SCLN, KC_COLN), //input ; :
	[UQUDQ]   = ACTION_TAP_DANCE_DOUBLE(KC_QUOT, KC_DQT),  //input ' "
	[UBSPI]   = ACTION_TAP_DANCE_DOUBLE(KC_BSLS, KC_PIPE), //input \ |
	[UF9LRP]  = ACTION_TAP_DANCE_FN_ADVANCED (NULL, dance_Uf9lrp_finished, dance_Uf9lrp_reset),//input F9 []
	[UF10LR]  = ACTION_TAP_DANCE_FN_ADVANCED (NULL, dance_Uf10lr_finished, dance_Uf10lr_reset) //input F10 []
};
